...
public class StoreOrderServiceImpl extends ServiceImpl<StoreOrderDao, StoreOrder> implements StoreOrderService {
    ...
    private List<StoreOrderItemResponse> formatOrder(List<StoreOrder> storeOrderList) {
        ...
        //门店id
        List<Integer> storeIdList = storeOrderList.stream()
            .map(StoreOrder::getStoreId)
            .distinct()
            .collect(Collectors.toList());
        //店员id / 核销员id
        List<Integer> clerkIdList = storeOrderList.stream()
            .map(StoreOrder::getClerkId)
            .distinct()
            .collect(Collectors.toList());

        //订单id集合
        List<Integer> orderIdList = storeOrderList.stream()
        ...
        //根据用户获取信息
        List<Integer> userIdList = storeOrderList.stream()
        ...
        for (StoreOrder storeOrder : storeOrderList) {
            ...
            //推广人信息
            if (!userList.isEmpty() && null != userList.get(storeOrder.getUid()) && mapListInUid.containsKey(userList.get(storeOrder.getUid())
                .getSpreadUid())) {
                storeOrderItemResponse.getSpreadInfo()
                    .setId(mapListInUid.get(userList.get(storeOrder.getUid()).getSpreadUid()).getUid());
                storeOrderItemResponse.getSpreadInfo()
                    .setName(mapListInUid.get(userList.get(storeOrder.getUid()).getSpreadUid()).getNickname());
            }
    ...
    public boolean refund(StoreOrderRefundRequest request) {
        ...
        //退款
        if (storeOrder.getPayType().equals(Constants.PAY_TYPE_WE_CHAT) && request.getAmount()
    ...
    public Boolean send(StoreOrderSendRequest request) {
        ...
        if (storeOrder.getIsDel()) {
            throw new CrmebException("订单已删除,不能发货!");
        }
        if (storeOrder.getStatus() > 0) {
            throw new CrmebException("订单已发货请勿重复操作!");
        }
    ...
    public LogisticsResultVo getLogisticsInfo(String orderNo) {
        ...
        return logisticService.info(info.getDeliveryId(), null, Optional.ofNullable(info.getDeliveryCode())
            .orElse(""), info.getUserPhone());
    }
    ...
    public StoreOrderStatisticsResponse orderStatisticsByTime(String dateLimit, Integer type) {
        ...
        if (type == 1) {
            ...
            } else if (perSumPrice == 0) {
                response.setGrowthRate((int)increasePrice * 100);
            } else {
                response.setGrowthRate((int)((increasePrice * perSumPrice) * 100));
            }
        } else if (type == 2) {
            ...
            if (increasePrice <= 0) {
                response.setGrowthRate(0);
            } else if (orderPerList.size() == 0) {
                response.setGrowthRate((int)increasePrice);
            } else {
                response.setGrowthRate((int)((increasePrice / orderPerList.size()) * 100));
            }
        }
    ...
    public Integer getLastProductOrderInfo(Integer productId, Integer uid) {
        ...
        return dao.getLastProductOrderInfo(dateLimit.getStartTime(), productId, uid);
    }

    @Override
    public Integer getLastOrderInfoByGift(Integer giftProperty, Integer uid) {
        DateLimitUtilVo dateLimit = DateUtil.getDateLimit(Constants.SEARCH_DATE_LATELY_30);
        return dao.getLastOrderInfoByGift(dateLimit.getStartTime(), giftProperty, uid);
    }
    ...
    private void express(StoreOrderSendRequest request, StoreOrder storeOrder) {
        ...
        String message = Constants.ORDER_LOG_MESSAGE_EXPRESS.replace("{deliveryName}", express.getName())
            .replace("{deliveryCode}", storeOrder.getDeliveryId());
    ...
    private void pushMessageOrder(StoreOrder storeOrder, User user, SystemNotification notification) {
        ...
        // 公众号
        if (storeOrder.getIsChannel().equals(Constants.ORDER_PAY_CHANNEL_PUBLIC) && notification.getIsWechat()
            .equals(1)) {
    ...
    private void delivery(StoreOrderSendRequest request, StoreOrder storeOrder) {
        if (StrUtil.isBlank(request.getDeliveryName())) {
            throw new CrmebException("请输入送货人姓名");
        }
        if (StrUtil.isBlank(request.getDeliveryTel())) {
            throw new CrmebException("请输入送货人电话号码");
        }
        ...
        String message = Constants.ORDER_LOG_MESSAGE_DELIVERY.replace("{deliveryName}", request.getDeliveryName())
            .replace("{deliveryCode}", request.getDeliveryTel());
        ...
        if (!execute) {
            throw new CrmebException("订单更新送货失败");
        }
